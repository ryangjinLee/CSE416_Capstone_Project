    ...
    <Table striped bordered hover className="text-center">
      <thead className="bg-light">
        <tr>
          ...
          <th colSpan="4" className="text-center">Race</th>
          <th colSpan="2" className="text-center">Party</th>
        </tr>
        <tr>
          <th>White</th>
          <th>Hispanic</th>
      ...
      <tbody>
        {data.map((item, index) => {
          const totalVotes = item.Republicans + item.Conservatives + item.Workers + item.Others + item.Blank;
          return (
            <tr key={index}>
